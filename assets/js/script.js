  ...
  confirm(
    ...
    return new Promise((resolve) => {
      ...
      overlay
        ...
        .addEventListener("click", () => handleAction(false));
      overlay
...
class Router {
  ...
  processRoute(hash) {
    const route =
      hash.replace("#/", "").split("?")[0] || ROUTER_CONFIG.defaultView;
  ...
  updateTitle(viewName) {
    document.title =
      ROUTER_CONFIG.titles[viewName] || "Doggo - No es un perro, es una movida";
  }
  ...
  // ============================================
     ...
     initPedidoForm() {
        ...
        // ❌ ELIMINAR: this.initClearCart(form);
        this.initFormSubmit(form);
        
        this.updateSummary();

        // ✅ DELEGACIÓN DE EVENTOS PARA VACIAR CARRITO
  ...
  initStepper(form) {
    ...
    indicators.forEach((indicator) => {
      indicator.addEventListener("click", () => {
        const step = parseInt(indicator.dataset.step);
        if (
          step < self.orderState.currentStep ||
  ...
  validateLeadFields(form) {
    ...
    if (!isValid) {
      ...
      const firstError = form.querySelector(
        '.step-content[data-step="1"] .error',
      );
      if (firstError) {
...
clearCart() {
    ...
    // 3. Limpiar el formulario visualmente
    const form = document.getElementById('pedido-form');
    if (form) {
        // Resetear inputs
        ...
        // Resetear cantidades de Bebidas
    ...
    // 5. ✅ FORZAR REINICIO DEL STEPPER
    const formElement = document.getElementById('pedido-form');
    if (formElement) {
        // ✅ Eliminar todas las clases active y completed
        formElement.querySelectorAll('.step-content').forEach(step => {
            step.classList.remove('active');
        });
        formElement.querySelectorAll('.step-indicator').forEach(indicator => {
            indicator.classList.remove('active', 'completed');
        });
        formElement.querySelectorAll('.step-line').forEach(line => {
            line.classList.remove('completed');
        });
        
        // ✅ Inicializar el stepper de nuevo
        this.initStepper(formElement);
    }
  ...
  initFormSubmit(form) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      if (nombreInput)
        this.orderState.leadData.nombre = nombreInput.value.trim();
      if (emailInput) this.orderState.leadData.email = emailInput.value.trim();
      if (whatsappInput)
        this.orderState.leadData.whatsapp = whatsappInput.value.trim();
      if (direccionInput)
        this.orderState.direccion = direccionInput.value.trim();
      if (referenciasInput)
        this.orderState.referencias = referenciasInput.value.trim();
      ...
      for (const [tipo, cantidad] of Object.entries(this.orderState.doggos)) {
        if (cantidad > 0) {
          ...
          total += subtotal;
          doggosList.push(
            `${cantidad}x ${tipo} ($${subtotal.toLocaleString()})`,
          );
        }
      }
      ...
      for (const [extra, cantidad] of Object.entries(this.orderState.extras)) {
        if (cantidad > 0) {
          ...
          total += subtotal;
          extrasList.push(
            `${cantidad}x ${extra} ($${subtotal.toLocaleString()})`,
          );
        }
      }

      let bebidasList = [];
      for (const [bebida, cantidad] of Object.entries(
        this.orderState.bebidas,
      )) {
        if (cantidad > 0) {
          ...
          total += subtotal;
          bebidasList.push(
            `${cantidad}x ${bebida} ($${subtotal.toLocaleString()})`,
          );
        }
      }
      ...
      try {
        ...
        Toast.success(
          "Tu pedido está en camino a WhatsApp",
          "¡Pedido enviado!",
          4000,
        );
  ...
  updateSummary() {
    ...
    if (doggoSummary) {
      const entries = Object.entries(this.orderState.doggos).filter(
        ([_, qty]) => qty > 0,
      );
      if (entries.length > 0) {
        let html = "";
    ...
    if (extrasSummary) {
      const entries = Object.entries(this.orderState.extras).filter(
        ([_, qty]) => qty > 0,
      );
      if (entries.length > 0) {
        let html = "";
    ...
    if (bebidasSummary) {
      const entries = Object.entries(this.orderState.bebidas).filter(
        ([_, qty]) => qty > 0,
      );
      if (entries.length > 0) {
        let html = "";
  ...
  loadOrderState() {
    try {
      ...
      if (saved) {
        ...
        this.orderState.leadData = parsed.leadData || {
          nombre: "",
          email: "",
          whatsapp: "",
        };
...
function addMicroInteractions() {
  ...
  document
    .querySelectorAll(".doggo-card, .extra-chip, .delivery-card, .bebida-card")
    .forEach((card) => {
