...
export class PedidoManager {
     ...
     */
    constructor(modalManager, toastManager) {
            ...
            const updateQuantity = (change) => {
                ...
                if (next > 0) {
                    ...
                    if (change > 0 && this.toastManager) {
                        this.toastManager.info(`${value} añadido (${next}x)`, 'Agregado', 1500);
                    }
                } else {
        ...
        cards.forEach(card => {
            card.addEventListener('click', () => {
                ...
                if (this.toastManager) {
                    const text = card.dataset.value === 'pickup' ? 'Recoger en local' : 'Envío a domicilio';
                    this.toastManager.info(`Opción seleccionada: ${text}`, 'Entrega', 2000);
                }
            });
    ...
    initDireccionFields() {
        ...
        if (referencias) {
            if (this.orderState.referencias) referencias.value = this.orderState.referencias;
    ...
    clearCart() {
        ...
        if (!form) {
            if (this.toastManager) {
                this.toastManager.success('Todos los datos han sido reiniciados.', 'Carrito vaciado');
            }
            return;
        }
    ...
    buildOrderSummary() {
        ...
        if (this.orderState.delivery === 'delivery') total += this.PRICES.delivery * 1000;
        ...
        return {
            ...
            deliveryCost: this.orderState.delivery === 'delivery' ? this.PRICES.delivery * 1000 : 0
        };
    }
...
Total: $${data.total.toLocaleString()}
    ...
    validateLeadFields() {
        ...
        if (!ok && this.toastManager) {
            this.toastManager.warning('Por favor, revisa tus datos.', 'Datos incompletos');
        }
    ...
    validateDoggos() {
        ...
        if (total <= 0) {
            if (this.toastManager) {
                this.toastManager.warning('Agrega al menos un Doggo para continuar.', 'Carrito vacío');
            }
            return false;
        }
        return true;
    ...
    validateStep3() {
        if (this.orderState.delivery === 'delivery') {
            ...
            if (!value) {
                if (this.toastManager) {
                    this.toastManager.warning('Por favor, ingresa la dirección de envío.', 'Dirección requerida');
                }
                direccion?.focus();
                return false;
            }
    ...
    updateSummary() {
        ...
        const summary = this.buildOrderSummary();
        delivery.innerHTML = `
            <div class="summary-item"><span class="summary-label">${summary.deliveryText}</span><span class="summary-value">${summary.deliveryCost ? `$${summary.deliveryCost.toLocaleString()}` : 'Gratis'}</span></div>
        ...
        totalBox.innerHTML = `
            <div class="total-breakdown">
                <div class="breakdown-item"><span>Subtotal</span><span>$${(summary.total - summary.deliveryCost).toLocaleString()}</span></div>
                <div class="breakdown-item"><span>Delivery</span><span>${summary.deliveryCost ? `$${summary.deliveryCost.toLocaleString()}` : 'Gratis'}</span></div>
