...
export class Router {
     ...
     */
    processRoute(hash) {
        const route = hash.replace('#/', '').split('?')[0] || this.config.defaultView;
    ...
    updateTitle(viewName) {
        document.title = this.config.titles[viewName] || 'Doggo - No es un perro, es una movida';
    }
    ...
    initViewComponents(viewName) {
        ...
        if (cleanViewName === 'pedido') {
            setTimeout(() => {
                this.pedidoManager = new PedidoManager(this.modalManager, this.toastManager);
